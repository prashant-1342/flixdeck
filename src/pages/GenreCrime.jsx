...
const GenreCrime = ({ searchQuery }) => {
  ...
  const fetchMovies = async (pageToLoad, query = '') => {
    ...
    try {
      const baseURL = query
        ...
        : `https://api.themoviedb.org/3/discover/movie?with_genres=80&language=en-US&page=${pageToLoad}`;
  ...
  return (
    <div className="pagescontainer2">
      <h2 className="heading2">{searchQuery ? `Search results for "${searchQuery}"` : 'Crime'}</h2>
